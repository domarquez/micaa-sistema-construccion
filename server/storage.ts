...
import { db } from "./db";
import { eq, desc, sql, like, ilike, and, gte, inArray } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async getBudgetItemsByBudgetId(budgetId: number): Promise<BudgetItem[]> {
  ...
  async getBudgetsByUserId(userId: number): Promise<any[]> {
    // Hacer JOIN entre budgets y projects para obtener información completa
    return await db
      .select({
        id: budgets.id,
