...
import type { ConstructionNews } from '@shared/schema';
...
export function HorizontalNewsTicker() {
  const [isPaused, setIsPaused] = useState(false);
  const tickerRef = useRef<HTMLDivElement>(null);

  // Fetch real news from the database
  const { data: news = [], isLoading } = useQuery<ConstructionNews[]>({
  ...
  const handleNewsClick = (newsItem: ConstructionNews) => {
      ...
      <div className="mobile-padding mb-2 sm:mb-3 md:mb-4">
        <div className="bg-blue-50 rounded-lg p-2 border border-blue-200">
          <div className="flex items-center gap-2">
            <Newspaper className="w-4 h-4 text-blue-600 animate-pulse" />
            <span className="text-sm text-blue-700">Cargando noticias...</span>
          </div>
        </div>
      </div>
    );
              ...
              >
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex items-center justify-between">
                    <Badge 
                      variant="outline" 
                      className={`text-[8px] sm:text-[9px] px-1 py-0 ${categoryColors[category]}`}
                    >
